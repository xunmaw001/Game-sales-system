<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="游戏名称" prop="youximingcheng">
          <el-select  @change="youximingchengChange" v-model="ruleForm.youximingcheng" placeholder="请选择游戏名称">
            <el-option
                v-for="(item,index) in youximingchengOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.youximingcheng" label="游戏名称" prop="youximingcheng">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="游戏分类" prop="youxifenlei">
          <el-input v-model="ruleForm.youxifenlei" 
              placeholder="游戏分类" clearable  :readonly="ro.youxifenlei"></el-input>
        </el-form-item>
        <div v-else>
...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.lianxidianhua!=''&&json.lianxidianhua){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/youxixinxi/youximingcheng`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.youximingchengOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    youximingchengChange () {
      this.$http({
        url: `follow/youxixinxi/youximingcheng?columnValue=`+ this.ruleForm.youximingcheng,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.youxifenlei){
                this.ruleForm.youxifenlei = data.data.youxifenlei
              }
        } else {
          this.$message.error(data.msg);
        }
